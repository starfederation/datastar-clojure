(ns examples.http-kit2.form-behavior
  (:require
    ...
    [starfederation.datastar.clojure.adapter.http-kit2 :refer [->sse-response on-open start-responding-middleware]]
  ...
  (rr/router
    [["/" {:handler form}]
     ["/endpoint"    {:handler process-form
                      :middleware [start-responding-middleware]}]
     c/datastar-route]))
